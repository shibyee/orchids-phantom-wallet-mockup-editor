          ...
          {screen === "s3" &&
            ...
            >
              <div className="flex items-center justify-between h-12 mb-6">
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="flex h-[30px] w-[30px] items-center justify-center rounded-full bg-[#2A2A2A] text-[14px] font-[800] text-white hover:bg-[#323232] transition-colors"
                  >
                    {data.badgeCount}
                  </button>
                  <div className="flex items-center gap-1 group cursor-pointer">
                    <span className="font-[800] text-[16px] tracking-tight">{data.homeName}</span>
              ...
              <div className="flex flex-col items-center mb-8">
                <div className="text-[44px] font-[600] leading-none mb-1 tracking-[-0.05em] font-phantom">
                  ${data.bal}
                </div>
                <div className="flex items-center gap-1.5 text-[15px] font-[700]">
                  <span className={data.delta.startsWith('-') ? 'text-[#ff5c5c]' : 'text-[#27c241]'}>
                    {data.delta.startsWith('-') ? '' : '+'}${data.delta}
                  </span>
                  <span className={`px-[6px] py-[2px] rounded-[5px] text-[13px] font-[800] ${data.pct.startsWith('-') ? 'bg-[#ff5c5c]/15 text-[#ff5c5c]' : 'bg-[#27c241]/15 text-[#27c241]'}`}>
              ...
              <div className="flex items-center justify-between rounded-[20px] bg-[#2A2A2A] p-[16px] mb-4 group cursor-pointer hover:bg-[#323232] transition-colors">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[16px] font-[700] text-white leading-tight tracking-tight">{data.tokName}</span>
                    <span className="text-[13px] font-[600] text-white/40 leading-tight">{data.tokAmt}</span>
                  </div>
                </div>
                <div className="flex flex-col items-end">
                  <span className="text-[16px] font-[700] text-white leading-tight tracking-tight">${data.tokUsd}</span>
                  <span className="text-[13px] font-[700] leading-tight text-[#ff5c5c]">
        ...
        <AnimatePresence mode="wait">
          {screen === "s1" &&
            ...
            >
              ...
              />
              <OptionButton
                icon={<Usb size={22} className="text-white/90" strokeWidth={1.5} />}
                title="Connect Hardware Wallet"
                sub="Use your Ledger hardware wallet"
              />
              <OptionButton
                icon={<FileText size={22} className="text-white/90" strokeWidth={1.5} />}
                title="Import Recovery Phrase"
                sub="Import accounts from another wallet"
              />
              <OptionButton
                icon={<ArrowDownToLine size={22} className="text-white/90" strokeWidth={1.5} />}
                title="Import Private Key"
                sub="Import a single-chain account"
                onClick={() => setScreen("s2")}
              />
              <OptionButton
                icon={<Eye size={22} className="text-white/90" strokeWidth={1.5} />}
                title="Watch Address"
                sub="Track any public wallet address"
              />
